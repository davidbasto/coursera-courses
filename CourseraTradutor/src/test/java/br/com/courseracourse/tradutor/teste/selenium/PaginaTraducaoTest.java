...
public class PaginaTraducaoTest {
	...
	private WebDriver buildWebDriver(String pathToDriver) {
		...
		try {
		    ...
		    waitUntilReady(marionettePort, TimeUnit.SECONDS.toMillis(5));
		} catch (Exception e) {
		    // ignore
		}
	...
	private void waitUntilReady(int webDriverPort, long millis) {
		boolean scanning=true;
		while(scanning)
		{
		    if(!available(webDriverPort)) {
		        try
		        {
		    	...
		    	System.out.println("Port Avaliable " + webDriverPort);
		        scanning=false;
		    }
		}
	}*/
